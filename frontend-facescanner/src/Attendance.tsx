...
function Attendance() {
  ...
  const [userEmailAddress, setEmailUserAddres] = useState('');
  ...
  useEffect(() => {
    Swal.fire({
      ...
      inputValidator: (value) => {
        if (!value) {
          return 'You Need To Input Email Address Here';
        }
      ...
      if (result.isConfirmed) {
        setEmailUserAddres(result.value);
        Swal.fire(`Email Stored Successfully`).then(() => {
          startCamera();
          selectedVenueRef.current = "BIL Workshop Room";
        });
      }
    });
    return () => {
  ...
  const captureAndMatchFace = () => {
    ...
    fetch('https://feasible-dove-simply.ngrok-free.app/facescanner', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ image: base64Image, event: selectedVenueRef.current, email: userEmailAddress }),
    })
      .then(res => res.json())
      .then(data => {
          ...
          <div className="flex items-center space-x-2 text-gray-600 text-sm">
            ...
            <select 
              ...
              onChange={(e) => {
                ...
                selectedVenueRef.current = e.target.value;
              }}
